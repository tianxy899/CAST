...
@register_model("hubert_transformer_d2s_gate")
class HubertTransformerModel(FairseqEncoderDecoderModel):
    ...
    def forward(self, src_tokens, src_lengths, mode, prev_output_tokens, start=None, x=None, encoder_padding_mask=None, is_single_input=False):
        """
        ...
        encoder_out = self.encoder(src_tokens=src_tokens, src_lengths=src_lengths, mode=mode, 
            start=start, x=x, encoder_padding_mask=encoder_padding_mask, is_single_input=is_single_input)
...
class HubertTransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary=None, embed_tokens=None):
        ...
        self.transformer_layers.extend([TransformerEncoderLayer(args) for _ in range(args.encoder_layers - 1)])
    ...
    def forward_audio(self, src_tokens, src_lengths):
        ...
        x, encoder_padding_mask = self.hubert_model.extract_features(**hubert_args)
        input_lengths = (1 - encoder_padding_mask.int()).sum(dim=1)
        ...
        return x, encoder_padding_mask, input_lengths, embed
    ...
    def forward_embedding(
        ...
        return x, encoder_padding_mask, embed

    def forward_mix(self, audio_emb, input_lengths, text_emb, text_lengths, start, extind, mix_ratio):
        final_length = input_lengths.new_zeros((input_lengths.size(0)))
        mix_tensors = []
        new_ind = []
        ...
        for i in range(audio_emb.size(0)):
            mix_lst = []
            length = 0
            num1, num2, num3 = torch.rand(1)[0], torch.rand(1)[0], torch.rand(1)[0]
            if num1 < mix_ratio:
                mix_lst.append(audio_emb[i, :extind[i, 0]])
                length += extind[i, 0]
            else:
                mix_lst.append(text_emb[i, :extind[i, 1]])
                length += extind[i, 1]
            if num2 < mix_ratio:
                mix_lst.append(audio_emb[i, extind[i, 0]:start[i, 0]])
                length += start[i, 0] - extind[i, 0]
            else:
                mix_lst.append(text_emb[i, extind[i, 1]:start[i, 1]])
                length += start[i, 1] - extind[i, 1]
            l1 = length.clone()
            new_ind.append(l1)
            if num3 < mix_ratio:
                mix_lst.append(audio_emb[i, start[i, 0]:input_lengths[i]])
                length += input_lengths[i] - start[i, 0]
            else:
                mix_lst.append(text_emb[i, start[i, 1]:text_lengths[i]])
                length += text_lengths[i] - start[i, 1]
            mix_tensors.append(mix_lst)
            final_length[i] = length

        max_len = max(final_length)
        x = audio_emb.new_zeros((audio_emb.size(0), max_len, audio_emb.size(2)))    # B x T x C
        for i in range(x.size(0)):
            x[i, :final_length[i]] = torch.cat(mix_tensors[i], 0)
            start[i, 1] = new_ind[i]

        encoder_padding_mask = lengths_to_padding_mask(final_length)
        if self.embed_positions is not None:
    ...
    def _forward(self, src_tokens, src_lengths, mode, start=None, x=None, encoder_padding_mask=None, is_single_input=False, return_all_hiddens=False):
        if x is None:   # inference
            if mode == "st":
                x, encoder_padding_mask, input_lengths, _ = self.forward_audio(src_tokens, src_lengths)
            else:
                has_pads = src_tokens.device.type == "xla" or encoder_padding_mask.any()
                x, encoder_padding_mask, _ = self.forward_embedding(src_tokens)
                if has_pads:
        ...
        cnt = 0 if mode == "st" else 1  # 0: audio; 1: text
        src_tags = encoder_padding_mask.clone()
        for i in range(start.size(0)):
            src_tags[i, :start[i][cnt]] = True

        for i, layer in enumerate(self.transformer_layers):
            if i == 0:    # first layers
                if is_single_input:
                    x, _ = layer(x, src_tags)
                else:
                    x, gate = layer(x, encoder_padding_mask, src_tags)     # gate: T x B x C
            else:
                x = layer(x, src_tags)
            if return_all_hiddens:
                encoder_states.append(x)
    ...
    def forward(self, src_tokens, src_lengths, mode, start=None, x=None, encoder_padding_mask=None, is_single_input=False, return_all_hiddens=False):
        x = self._forward(
            src_tokens, src_lengths, mode, start=start, x=x, encoder_padding_mask=encoder_padding_mask, 
            is_single_input=is_single_input, return_all_hiddens=return_all_hiddens
        )
        return x
    ...
    def reorder_encoder_out(self, encoder_out, new_order):
        ...
        new_gate = (
            encoder_out["gate"]
            if encoder_out["gate"] is None
            else encoder_out["gate"].index_select(1, new_order)
        )
    ...
    def max_positions(self):
        ...
        return 1e7  # an arbitrary large number

@register_model_architecture(model_name="hubert_transformer_d2s_gate", arch_name="hubert_transformer_d2s_gate")
...
@register_model_architecture(model_name="hubert_transformer_d2s_gate", arch_name="hubert_transformer_d2s_gate_postln")
