...
import cress.datasets.speech_and_text_translation_dataset_d2s_gate as gate
...
class SpeechAndTextTranslationTask(LegacyFairseqTask):
    ...
    def add_args(cls, parser):
        ...
        parser.add_argument('--doc-mode', action='store_true',
                            help='use doc2sent model')
        parser.add_argument('--doc-mode-gate', action='store_true',
                            help='use doc2sent model')
    ...
    def __init__(self, args, src_dict, tgt_dict):
        ...
        if getattr(args, "doc_mode_gate", False) is True:
            self.SpeechAndTextTranslationDataset = gate.SpeechAndTextTranslationDataset
            self.SpeechAndTextTranslationDatasetCreator = gate.SpeechAndTextTranslationDatasetCreator
        else:
            self.SpeechAndTextTranslationDataset = SpeechAndTextTranslationDataset
            self.SpeechAndTextTranslationDatasetCreator = SpeechAndTextTranslationDatasetCreator
    ...
    def load_st_dataset(self, split, epoch):
        is_train_split = split.startswith("train")
        return self.SpeechAndTextTranslationDatasetCreator.from_tsv(
            self.args.data,
            self.data_cfg,
            split,
            self.tgt_dict,
    ...
    def build_dataset_for_inference(self, src_tokens, src_lengths, **kwargs):
        return self.SpeechAndTextTranslationDataset(
    ...
    def inference_step(
        ...
        if getattr(self.args, "debug_task", "st") == "st" and "audio" in sample["net_input"]:
            net_input = {
                ...
                # "start": sample["start"],     # sentence-level: none
            }
