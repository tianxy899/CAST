...
class GTransformerEncoderLayerBase(nn.Module):
    """Encoder layer block.
    ...
    *cfg.encoder.normalize_before* to ``True``.
    ...
    def __init__(self, cfg, return_fc=False):
        ...
        self.self_attn_gate = nn.Sequential(Linear(self.embed_dim * 2, 1), nn.Sigmoid())
    ...
    def forward(
        self,
        x,
        encoder_padding_mask_global,
        encoder_padding_mask_local: Optional[Tensor] = None,
        ...
        x_global, _ = self.self_attn(
            ...
            key=x,
            value=x,
            key_padding_mask=encoder_padding_mask_global,
            need_weights=False,
            attn_mask=attn_mask,
        )
        g = None
        if encoder_padding_mask_local is None:
            x = x_global    # 3句完整，或3句的最后一句
        else:
            x_local, _ = self.self_attn_local(
                query=x,
                key=x,
                value=x,
                key_padding_mask=encoder_padding_mask_local,
                need_weights=False,
                attn_mask=attn_mask,
            )
...
class GTransformerEncoderLayer(GTransformerEncoderLayerBase):
...
# backward compatible with the legacy argparse format
class GTransformerDecoderLayer(GTransformerDecoderLayerBase):
    def __init__(
